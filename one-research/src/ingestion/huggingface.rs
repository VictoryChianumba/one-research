...
/// results at `~/.config/one-research/hf_repo_cache.json` (7-day TTL) loaded
...
/// `HuggingFaceSource::fetch` is the only `Source` impl in one-research that
...
/// site in one-research. Pre-C10 this lived in a local `fetch_arxiv_with_retry`
/// helper (commit `5491470`); the audit's C13 lifted it into `one_research_http`.
fn fetch_abstracts(items: &mut [FeedItem], ctx: &FetchContext) {
  ...
  let body = match one_research_http::read_body(resp) {
    Ok(b) => b,
    Err(e) => {
...
///
/// Results are cached at `~/.config/one-research/hf_repo_cache.json` with a
