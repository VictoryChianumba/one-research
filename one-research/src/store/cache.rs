...
pub fn load() -> Vec<FeedItem> {
  ...
  let mut items: Vec<FeedItem> = super::load_json(&path, "one-research/cache");
...
pub fn save(items: &[FeedItem]) {
  ...
  super::save_json(&items, &path, "one-research/cache");
    ...
    let _ = thread::Builder::new()
      ...
      .spawn(move || {
        ...
        if let Err(payload) = result {
          ...
          log::error!(
            "one-research-cache-writer thread panicked: {msg} — subsequent \
...
pub fn flush_blocking() {
  ...
  match writer_handle().send(WriterMsg::Flush(ack_tx)) {
    Err(_) => {
      log::error!(
        "one-research-cache-writer: flush send failed — writer thread is gone, \
    ...
    Ok(()) => {
      if ack_rx.recv_timeout(Duration::from_secs(5)).is_err() {
        log::error!(
          "one-research-cache-writer: flush ack timeout (5s) — disk wedged or \
...
mod tests {
  ...
  fn load_sanitizes_escape_sequences_in_existing_cache() {
    ...
    let dir = std::env::temp_dir()
      .join(format!("one_research_cache_load_test_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(dir.join(".config/one-research")).unwrap();
    std::fs::write(dir.join(".config/one-research/cache.json"), &json).unwrap();
