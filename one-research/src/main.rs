...
fn migrate_legacy_config_dir() {
  ...
  for legacy in ["tentative", "one-research"] {
    ...
    if let Err(e) = std::fs::create_dir_all(&new_root) {
      eprintln!("one-research: could not prepare config dir ({e}); continuing");
      return;
    }
    ...
    for name in [
      ...
      // access to the legacy dir, planting `state.json` as a symlink pointing
      // to a victim file would let our rename move that file unexpectedly.
      // Skip symlinks; the legacy dir is ephemeral and a subsequent launch
      // can retry.
      match std::fs::symlink_metadata(&old_path) {
        Ok(m) if m.file_type().is_symlink() => {
          eprintln!(
            "one-research: refusing to migrate symlink at {}; skipping",
            old_path.display()
          );
          continue;
        }
      ...
      if let Err(e) = std::fs::rename(&old_path, &new_path) {
        eprintln!(
          "one-research: could not migrate {} to new config dir ({e}); continuing",
          old_path.display()
        );
      }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  // one-research (its `init()` is only invoked from tread's standalone main).
  // Cheap no-op when the env var isn't set.
  tread::bench::init();
  tread::bench::emit_us("one_research_main_entered", 0);
  ...
  let log_level = if std::env::var_os("ONE_RESEARCH_DEBUG_LOG").is_some() {
    log::LevelFilter::Debug
  } else {
    log::LevelFilter::Info
  };
  let log_file = dirs::home_dir().and_then(|home| {
    let path = home.join(".config/one-research/one-research.log");
    std::fs::create_dir_all(path.parent()?).ok()?;
    // Rotate on startup: move existing one-research.log → one-research.log.1 so the
    ...
    // Owner-read/write only on Unix. Default umask leaves one-research.log
  ...
  // know that cache.json, history, chat sessions, notes, and one-research.log
  ...
  #[cfg(windows)]
  log::warn!(
    "one-research on Windows: data files use default ACLs and may be \
  ...
  // pane stays blank — but one-research has no other way to tell the user
  // that's not a one-research bug.  Mirrors the standalone tread warning.
  if tread::in_zellij() && tread::is_iterm2() && tread::detect_kitty_supported()
  {
    eprintln!(
      "one-research: zellij over iTerm2 detected. figure-preview panes may\n  \
       ...
       - run one-research outside Zellij\n    \
  ...
  {
    eprintln!(
      "one-research: tmux detected.  If figures don't render, ensure tmux\n  \
  ...
  // One Research's existing keys.rs already uses `KeyCode::Enter` (not
  ...
  // a single INFO log line so the distribution is visible in one-research.log
  ...
  let run_result: std::io::Result<()> = (|| -> std::io::Result<()> {
    loop {
      ...
      // one-research's dirty flag so the next frame redraws.
      ...
      if let Some(rx) = app.async_jobs.tread_fetch_rx.as_ref() {
        match rx.try_recv() {
          Ok(result) => {
            ...
            tread::bench::emit_us("one_research_tread_drain", 0);
            ...
            tread::bench::emit_us("one_research_reader_opened", 0);
            app.clear_notification();
            app.mark_dirty();
          }
          Err(std::sync::mpsc::TryRecvError::Disconnected) => {
