...
impl FetchContext<'_> {
  /// Access the process-wide HTTP client. Re-exports `trench_http::client()`
  /// so source impls never need to import the http crate directly — keeps
  /// tripwire J4 simple ("no `crate::http::client()` in `trench/src/ingestion/`").
  pub fn http(&self) -> &'static reqwest::blocking::Client {
    crate::http::client()
  }

  /// Issue a request with retry semantics per `policy`. Forwards to
  /// [`trench_http::with_retry`]. Use `RetryPolicy::arxiv()` for sources
  ...
  {
    trench_http::with_retry(self.http(), policy, make)
  }
}
...
mod tests {
  ...
  fn http_returns_the_singleton() {
    // `ctx.http()` and `trench_http::client()` must alias — sources
    ...
    assert!(
      std::ptr::eq(ctx.http(), trench_http::client()),
