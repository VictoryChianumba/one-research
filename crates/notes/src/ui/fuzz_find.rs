...
impl FilteredEntry {
  fn new(id: String, score: i64, indices: Vec<usize>) -> Self {
...
impl FuzzFindPopup<'_> {
  ...
  fn render_entries_list(&mut self, frame: &mut Frame, area: Rect) {
    let items: Vec<ListItem> = self
      ...
      .map(|entry| {
        let entry_title =
          self.entries.get(&entry.id).expect("Entry must be in entries map");
    ...
    let block_title = format!("Notes: {}", self.filtered_entries.len());
  ...
  pub fn handle_input(&mut self, key: KeyEvent) -> FuzzFindReturn {
    ...
    match key.code {
      ...
      _ => {
        if self.query_text_box.input(key) {
  ...
  pub fn cycle_prev_entry(&mut self) {
    ...
    let new_index =
      self.list_state.selected().map_or(0, |idx| idx.saturating_sub(1));
  ...
  fn update_search_query(&mut self) {
    ...
    let query_text =
      self.query_text_box.lines().first().expect("Query text box has one line");

    self.search_query =
      if query_text.is_empty() { None } else { Some(query_text.to_owned()) };
    ...
    if let Some(query) = self.search_query.as_ref() {
      self.filtered_entries = self
        ...
        .iter()
        .filter_map(|(id, title)| {
          self.matcher.fuzzy_indices(title, query).map(|(score, indices)| {
            FilteredEntry::new(id.clone(), score, indices)
          })
        })
        .collect();
