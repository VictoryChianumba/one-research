...
pub type SortReturn = PopupReturn<SortResult>;
...
impl SortPopup {
  pub fn new(sorter: &Sorter) -> Self {
    let mut sort_popup = Self {
      ...
      active_control: SortControl::AvailableList,
      available_state: Default::default(),
      applied_state: Default::default(),
      is_valid: true,
    };

    sort_popup.load_from_sorter(sorter);
    sort_popup.validate();

    sort_popup
  }

  fn load_from_sorter(&mut self, sorter: &Sorter) {
  ...
  fn render_applied_items(&mut self, frame: &mut Frame, area: Rect) {
    ...
    let list_block =
      Block::default().borders(Borders::ALL).title(title).style(block_style);
  ...
  pub fn handle_input(&mut self, key: KeyEvent) -> SortReturn {
    ...
    match key.code {
      ...
      KeyCode::Char('k') | KeyCode::Up => match self.active_control {
        SortControl::AvailableList => Self::cycle_prev_criteria(
          &self.available_criteria,
          &mut self.available_state,
        ),
        SortControl::AppliedList => Self::cycle_prev_criteria(
          &self.applied_criteria,
          &mut self.applied_state,
        ),
      },
      KeyCode::Char('j') | KeyCode::Down => match self.active_control {
        SortControl::AvailableList => Self::cycle_next_criteria(
          &self.available_criteria,
          &mut self.available_state,
        ),
        SortControl::AppliedList => Self::cycle_next_criteria(
          &self.applied_criteria,
          &mut self.applied_state,
        ),
      },
      ...
      KeyCode::Char('d') if has_control => {
        self.load_from_sorter(&Sorter::default())
      }
      _ => {}
    };
  ...
  fn cycle_next_criteria(criteria: &[SortCriteria], state: &mut ListState) {
    ...
    let new_idx = state
      .selected()
      .map(|idx| if idx >= criteria.len() - 1 { 0 } else { idx + 1 });
  ...
  fn move_criteria_up(&mut self) {
    if self.applied_criteria.is_empty()
      || self.applied_state.selected().is_none()
    {
      return;
    }
  ...
  fn confirm(&mut self) -> SortReturn {
    ...
    let result = SortResult {
      applied_criteria: self.applied_criteria.clone(),
  ...
  fn get_footer_text(&self) -> String {
    let move_mapping = match self.active_control {
      SortControl::AvailableList => "",
      SortControl::AppliedList => {
        " <Ctrl-j/k> or <Ctrl-Up/Down> Move criteria up/down |"
      }
    };
