...
| Term | Meaning |
|------|---------|
| **FeedItem** | The canonical record for one piece of content (paper, article, repo, digest, thread). Lives in `one-research/src/models/`. Carries source, signal, workflow state, summary, URL. |
...
| **ReaderInstanceModel** | One embedded reader pane (primary or secondary), holding the single open paper as `doc: Option<ReaderTab>`. The `ReaderTab` wraps a `tread::Reader` plus its image cache. Introduced by slice 2 (`ADR-002`); document tabs were removed (`ADR-017`) — one paper at a time, opening another replaces it. |
...
| **NotesInstanceModel** | One notes context (primary or secondary). Owns `NotesMode`, the optional `NotesContext` (paper anchoring), and the browser `selected` note (`ADR-016`). One note open at a time — document tabs were removed (`ADR-017`). Per-instance — primary and secondary can be in different modes and tied to different papers. Pure content; visibility lives on the parent `NotesPaneModel`. Selection is a `note_id` reconciled against an orchestrator-supplied visible set, so the dock — not the backend's `current_note_id` — is the single owner of selection. |
...
| **BrowseModel** | Composition-root model for the Subject Browser tab (`FeedTab::Browse`, `ADR-010` + `ADR-011`). Owns the right-rail drill stack (`rail_path`), one `rail_cursor`, rail-vs-feed focus, session-scoped per-category paging buffers (`loaded_categories: HashMap<String, CategoryBuffer>`), and the inflight-fetch set (`inflight`). Sibling to `DiscoveryModel`. Lives at `one-research/src/app/state/browse.rs`. |
| **Subject Browser** | The `FeedTab::Browse` surface (`ADR-010` + `ADR-011`). The main feed table stays on the left while a Details-width right companion pane shows a replace-mode arXiv rail (Groups -> Archives -> Categories). `l` enters/drills rightward, `h` backs out leftward, `j`/`k` move within a level, `Enter` fetches a Category, and `p` promotes a category into `config.sources.arxiv_categories`. While filter focus is active that right pane swaps from the rail to Filters. Replaces the deleted `KNOWN_ARXIV_CATS` shortlist as the arXiv-category management surface. |
| **Group / Archive / Category** | The three levels of arXiv's [official taxonomy](https://arxiv.org/category_taxonomy). **Group** = top-level subject area (8 today: Physics, Mathematics, CS, Quant-Bio, Quant-Fin, Stats, EESS, Economics). **Archive** = submission stream under a group (e.g. `astro-ph` under Physics; the CS group has just one archive, `cs`). **Category** = leaf code with a canonical form like `cs.LG` or a bare archive id like `gr-qc` for archives without subcategories. Encoded as `&'static` slices in `one-research/src/models/arxiv_taxonomy.rs::TAXONOMY` (~155 categories). Read by the `BrowseModel` renderer; not consulted in the bulk-refresh path. |
| **CategoryBuffer** | Per-category paging buffer (`ADR-015` §F1) held in `BrowseModel.loaded_categories`. Carries the fetched `urls` (resolved against `items_store`), the `next_offset` for the following arXiv page, and an `exhausted` flag set when a short page (`< BROWSE_PAGE_SIZE`) returns. PR 1 renders the URL count as an honest per-category indicator in the rail; PR 2 reads `next_offset` / `exhausted` to page deeper on scroll. Supersedes ADR-010's flat `Vec<url>`. Lives at `one-research/src/app/state/browse.rs`. |
| **Source** | Trait implemented by bulk-ingestion modules (`ArxivSource`, `HuggingFaceSource`, `RssSource`, `OpenReviewSource`, `CoreSource`). One `fetch(&FetchContext) -> Result<Vec<FeedItem>>` method plus `name()` + `host_group()` for the orchestrator's scheduling. Lives in `one-research/src/ingestion/pipeline.rs`. Introduced by C10 (`ADR-004`). |
...
| **FetchContext** | Per-refresh context passed to every `Source::fetch` / `EnrichmentSource::enrich` call. Carries `&Config`, `&Path` (cache_dir), and convenience methods `http()` and `with_retry(policy, make)` that forward to `one_research_http`. |
