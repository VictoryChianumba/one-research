...
| **NotesInstanceModel** | One notes context (primary or secondary). Owns tabs, active tab, `NotesMode`, the optional `NotesContext` (paper anchoring), and the browser `selected` note (`ADR-016`). Per-instance — primary and secondary can be in different modes and tied to different papers. Pure content; visibility lives on the parent `NotesPaneModel`. Selection is a `note_id` reconciled against an orchestrator-supplied visible set, so the dock — not the backend's `current_note_id` — is the single owner of selection. |
