...
| **FrameLayout** | C6 in flight (2026-05-18) | ADR-008. PR 1 lands `FrameLayout` struct + empty `App::apply_frame_layout` hook + ADR + smoke tests. PR 2 wires the hook + migrates the last "Intentional render-time mutation" marker (reader-bottom feed-mode auto-scroll). PR 3 tripwires N1-N3. Trigger: audit candidate C6 — single forcing function (the marker block) names this as ready. |
...
- `docs/adr/ADR-008-frame-layout.md` — C6 `FrameLayout` + `apply_frame_layout` hook (Proposed 2026-05-18; PR 1 landed).
