...
| **BrowseModel** | Composition-root model for the Subject Browser tab (`FeedTab::Browse`, `ADR-010`). Owns four column cursors (`groups`, `archives`, `categories`, `recent`) over the static arXiv taxonomy, the focused-column index, and the session-scoped per-category URL lists (`loaded_categories`) + inflight-fetch set (`inflight`). Sibling to `DiscoveryModel`. Lives at `trench/src/app/state/browse.rs`. |
| **Subject Browser** | The `FeedTab::Browse` surface (`ADR-010`). A 4-column Miller-layout navigator over arXiv's full taxonomy (Groups | Archives | Categories | Recent). `h`/`l` shift focused column; `j`/`k` move within. PR 2 wires `Enter` on a Category to spawn a per-category fetch; PR 3 wires `p` to promote a category into `config.sources.arxiv_categories`. Replaces the deleted `KNOWN_ARXIV_CATS` shortlist as the arXiv-category management surface. |
