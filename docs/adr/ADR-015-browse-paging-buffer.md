...
- **R1. Rate-limit throttling under fast scroll / fast rail navigation.** Mitigation: `inflight` + `exhausted` gating (§F4), arrival debounce (§F5). If throttling still appears, the page size and debounce window are the tuning knobs; the `with_retry` envelope is the escalation path.
  - **Materialised + hardened (2026-06-03).** arXiv throttles with an **HTTP 200** plain-text `Rate exceeded.` body, which `parse_atom` was reading as an empty feed → the Browse pipeline cached the category as a 0-paper *exhausted* buffer ("0 papers loaded", stuck). `parse_atom` now rejects any non-`<feed>` body as `Err`, so a throttle routes to `BrowseMessage::Error` (inflight cleared, nothing cached, retryable via `Enter`) and surfaces the reason. The `autofill_attempted` guard (R6) is deliberately *kept* on error — re-arming auto-fill would re-fire every poll while the cursor rests on a throttled category, a self-perpetuating storm. **Still open:** the underlying request *cadence* (auto-fill + paging + `Enter` + the concurrent bulk refresh) routinely exceeds arXiv's ~1-req/3s limit on a fresh launch; an app-wide arXiv request throttle / `with_retry` backoff is the real escalation (out of scope here, tracked as the §F5 tuning + R1 follow-up).
