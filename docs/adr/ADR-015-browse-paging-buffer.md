...
- **R1. Rate-limit throttling under fast scroll / fast rail navigation.** Mitigation: `inflight` + `exhausted` gating (§F4), arrival debounce (§F5). If throttling still appears, the page size and debounce window are the tuning knobs; the `with_retry` envelope is the escalation path.
  - **Materialised + hardened (2026-06-03).** arXiv throttles with an **HTTP 200** plain-text `Rate exceeded.` body, which `parse_atom` was reading as an empty feed → the Browse pipeline cached the category as a 0-paper *exhausted* buffer ("0 papers loaded", stuck). `parse_atom` now rejects any non-`<feed>` body as `Err`, so a throttle routes to `BrowseMessage::Error` (inflight cleared, nothing cached, retryable via `Enter`) and surfaces the reason. The `autofill_attempted` guard (R6) is deliberately *kept* on error — re-arming auto-fill would re-fire every poll while the cursor rests on a throttled category, a self-perpetuating storm.
  - **Prevention added (2026-06-03).** The recovery above is belt; this is suspenders. A process-global gate, `arxiv::throttle()`, now fronts **every** live `export.arxiv.org` request — bulk refresh, Browse paging + arrival auto-fill, the search bar, and HF abstract enrichment (which also hits the arXiv API). It blocks each caller until ≥3s have elapsed since the previous arXiv request (a global `Mutex<Option<Instant>>` held across the sleep → a leaky bucket of one), so the independent worker threads can no longer collectively burst past arXiv's ~1-req/3s envelope. Host-group serialization (ADR-004) only spaced requests *within* one bulk-refresh thread; this closes the cross-thread gap that was the actual cause. Interval is the tuning knob (`ARXIV_MIN_REQUEST_INTERVAL`, 3000ms). The gate runs on worker threads only — never the UI/render thread — so pacing never stalls the event loop. Pure spacing policy unit-tested via `arxiv_wait_for`. **TUI-verified 2026-06-08** on a recovered IP: three queued auto-fills (cs.AR/cs.CC/cs.CE) completed serialized ~3s apart (+1s/+4s/+7s), no rate-limit/timeout.
  - **Backoff added (2026-06-08).** Pacing alone can't rescue an *already*-penalised IP: arXiv's per-IP throttle is sticky (observed ~5 days to clear after heavy abuse, and it discriminates by load — a `max_results=1` health-check can pass while real `max_results=50` queries still time out). So the gate now also carries a cooldown: on any sign of pushback — a `Rate exceeded.` / non-feed body *or* a connection timeout (`error sending request`, which is arXiv dropping the connection before it even 429s) — `arxiv::enter_cooldown()` holds **all** arXiv callers off for `ARXIV_COOLDOWN` (60s) instead of letting them keep poking every 3s and sustaining the penalty. The four arXiv free functions now route through one `fetch_atom` chokepoint (gate → send → read → parse → arm-cooldown-on-pushback) so the throttle and backoff can't be forgotten at a call site; the HF enrichment path arms the cooldown on its own failure. Pure cooldown policy unit-tested via `arxiv_gate_wait`. The cooldown duration is the tuning knob. *Caveat:* the cooldown's end-to-end effect (a penalised IP recovering faster because the app went quiet) is not directly verifiable without deliberately throttling the IP — declined, to avoid re-burning it.
