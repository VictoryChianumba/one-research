...
- **Status:** Proposed (2026-05-19). PR 1 (this ADR + `BrowseModel` rail refactor + single-column drill UI + tab-order swap putting Browse first + cycle-test updates). PR 2 (sort modes `dated`/`random`/`popular`/`trending` + subject-follow toggle wired through filter pane + `F` quick-toggle). PR 3 (Subject column in Browse feed table + extend `scripts/check-subject-browser.sh` with P1-P5 ADR-011 invariants + flip Status to Accepted).
- **Date:** 2026-05-19
