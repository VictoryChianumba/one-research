...
- **Status:** Proposed (2026-05-19). PR 1 ships ADR + `DebounceState` + 5 smoke tests + 4-field migration + CONTEXT.md vocabulary entry.
- **Date:** 2026-05-19
...
| Cluster | Fields | Struct |
|---|---|---|
| Leader key | `leader_active`, `leader_activated_at`, `leader_timeout_ms` (3) | `LeaderState` |
| Reader-bottom drawer | `reader_bottom_open`, `reader_bottom_focused`, `reader_bottom_details`, `reader_feed_popup_selected`, `reader_bottom_scroll` (5) | `ReaderBottomState` |
| Async fetch receivers | `fetch_rx`, `fulltext_rx`, `tread_fetch_rx`, `repo_fetch_rx`, `is_loading`, `loading_sources`, `loaded_sources`, `spinner_frame`, `fulltext_loading`, `pending_fulltext_context`, `pending_tread_fetch` (11) | `AsyncJobs` |
| View-state popups | `tab_window_prompt_active`, `abstract_popup_active`, `narrow_feed_details_open`, `fulltext_new_tab`, `fulltext_for_secondary` (5) | `ViewFlags` |
| RefCell caches | `visible_cache`, `counts_cache`, `filter_source_names_cache`, `filter_summary_cache`, `filtered_history_cache` (5) | `RenderCaches` |
