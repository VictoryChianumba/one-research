...
- **Status:** Proposed (2026-05-22). PR 1 + PR 2 of 3 landed. PR 1: `nucleo` dependency + standalone `search::engine::FeedSearch` (a `Nucleo<u32>` worker with the four-column grammar mapping below) + 3 engine tests. PR 2: wired into the feed pipeline — lazy `App.feed_search` (created on first search char, dropped on clear), corpus injection on activation + on `ItemsChanged`, pattern driven from the search-bar chokepoints (`push`/`pop`/`clear`), `visible_indices_for` consumes the ranked snapshot with `cat:`/`year:` gates + substring fallback, frame-loop `tick` invalidates the visible cache while results stream, and `fuzzy-matcher` + the synchronous `Query::score` are removed (`Query` keeps parsing + `passes_gates`/`matches_substring`). PR 3 remaining: loop-cadence refinement (only stay hot while the worker is running), incremental delta injection instead of full reload, and final help/README polish.
- **Date:** 2026-05-22
