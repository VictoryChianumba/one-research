...
- **Status:** Proposed (2026-05-22). PR 1 of 3 landed: `nucleo` dependency + standalone `search::engine::FeedSearch` (a `Nucleo<u32>` worker with the four-column grammar mapping below) + 3 inline engine tests proving cross-field matching, field-scoping, and title-over-abstract ranking. Not yet wired into the feed pipeline — the synchronous ADR-012 `Query` scorer remains the live path until PR 2.
- **Date:** 2026-05-22
